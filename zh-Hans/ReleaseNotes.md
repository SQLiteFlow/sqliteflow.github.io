...
6.8.7 - 2026年8月6日
- 转储数据库。移除选项“仅数据”。增加选项“包含DDL"和“包含数据”。
- 将SQLite库更新到3.53.4版本（2026-07-24）。
- 其它问题修复与稳定性改善。
...
6.8.5 - 2026年7月10日
- 将SQLite库更新到3.53.3版本（2026-06-26）。
- 将SQLCipher更新到4.17.0。
- 其它问题修复与稳定性改善。

6.8.4 - 2026年5月16日
- 将SQLite库更新到3.53.1版本（2026-05-05）。
- 将SQLCipher更新到4.16.0。
- 数据库列表，支持右键点击一个表名并使用ALTER TABLE ADD COLUMN语句来添加一个字段。
- 数据库列表，支持右键点击一个字段名并使用ALTER TABLE DROP COLUMN语句来删除一个字段。

6.8.3 - 2026年4月10日
- 将SQLite库更新到3.53.0版本（2026-04-09）。
- 打开数据库对话框现在将默认显示选项视图。
- 打开数据库对话框现在会保存“允许选择所有文件类型”选项。

6.8.1 - 2026年3月14日
- 更新SQLite版本到3.51.3 (2026-03-13)。
- 修改表结构。添加列时避免生成同样的新列名。
- 修改表结构。当从UI移除列时，如果该列包含单列唯一约束，该约束会被一并移除，以避免后续修改表结构是产生语法错误。

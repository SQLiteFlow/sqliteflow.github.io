...
6.8.7 - 2026年8月6日
- ダンプデータベース。「データのみ」オプションを削除してください。「DDL を含める」と「データを含める」の2つのオプションを追加してください。
- SQLite ライブラリをバージョン 3.53.4（2026-07-24）に更新する。
- 追加のバグ修正と安定性の向上。

6.8.5 - 2026年7月10日
- SQLite ライブラリをバージョン 3.53.3（2026-06-26）に更新する。
- SQLCipher を 4.17.0 に更新してください。
- 追加のバグ修正と安定性の改善。

6.8.4 - 2026年5月16日
- SQLite ライブラリをバージョン 3.53.1（2026-05-05）に更新する。
- SQLCipher を 4.16.0 に更新してください。
- データベースリストは、ALTER TABLE ADD COLUMN 文を使用してテーブル名を右クリックしてテーブルフィールドを追加することをサポートします。
- データベースリストは、ALTER TABLE DROP COLUMN 文を使用してフィールド名を右クリックしてテーブルフィールドをドロップすることをサポートします。

6.8.3 - 2026年4月10日
- SQLite ライブラリをバージョン 3.53.0（2026-04-09）に更新する。
- データベース ダイアログを開くと、デフォルトでオプションビューが表示されます。
- データベースを開くダイアログは、オプション「すべてのファイルの種類を選択できるようにする」を永続化できるようになりました。

6.8.1 - 2026年3月14日
- SQLite ライブラリをバージョン 3.51.3（2026-03-13）に更新する。
- テーブルスキーマ。新しいフィールドを追加する際に、同じデフォルトの新しいフィールド名を生成しないようにしてください。
- テーブルスキーマ。UI で列を削除する際、その列に単一列の一意の制約がある場合、その一意の制約はこの列と同時に削除され、テーブル文法のさらなる変更エラーを防止します。

6.8.0 - 2026年3月8日
- データエディタ。ドラッグアンドドロップでフィルタ行を再配置することをサポートします。
- SQLite バージョン 3.52.0（2026-03-06）をサポートしています。
- SQLCipher を 4.13.0 に更新する。

...
6.3.3 - 2024年03月31日
- データベースER図の表示をサポートします。メニュー - データベース - ER図で表示できます。
- ER図は、トグル表示または非表示フィールドタイプをサポートします。
- ER図は、関係推測の有効化をサポートします。この設定が有効な場合、ダイアグラムは、2 つのテーブルが適切なテーブル名とフィールド名を持っている場合、リレーションシップを持つと宣言されていなくても、リレーションシップを持つものとして扱われます。たとえば、Customer テーブルと Invoice テーブル (CustomerID という名前のフィールドを持つ) は、テーブル Invoice に外部キー宣言がない場合でも、リレーションシップを持つものとして扱われます。
- ER図は、マーメイドマークダウンとしてのコピーをサポートします。

...
- 將SQLite庫更新到3.53.4版本（2026-07-24）。
- 其它問題修復與穩定性改善。

6.8.5 - 2026年7月10日
- 將SQLite庫更新到3.53.3版本（2026-06-26）。
- 將SQLCipher更新到4.17.0。
- 其它問題修復與穩定性改善。
...
- 打開數據庫對話框現在將默認顯示選項視圖。
- 打開數據庫對話框現在會保存「允許選擇所有文件類型」選項。

6.8.1 - 2026年3月14日
- 更新SQLite版本到3.51.3 (2026-03-13)。
- 修改表結構。添加列時避免生成同樣的新列名。
- 修改表結構。當從UI移除列時，如果該列包含單列唯一約束，該約束會被一併移除，以避免後續修改表結構是產生語法錯誤。
